...
class Predict(OP):
    ...
    def execute(self, op_in: OPIO) -> OPIO:
        ...
        return OPIO({
            "predict_result": Path(op_in["predict_code"])/'test_results.csv' # /'id_prop.csv'
        })
...
def main():
    ...
    for i, criteria in enumerate(screen_name_ls):
        ...
        step_screen =  Step(
            ...
            parameters = {"dirct": screen_name_ls[i][:-3], "api": 'v8mU74QhZSIisN26'},
        )
    ...
    predict = Step(
        "predict",
        PythonOPTemplate(Predict, image = 'kianpu/cgcnn:v1.1.0',),
